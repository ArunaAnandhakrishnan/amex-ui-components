...
import { AmexPageHeaderComponent, AmexBreadcrumbTrailComponent, AmexStatusBadgeComponent } from '@vn-core-ui-components/ui';
...
@Component({
      ...
      <div *ngIf="selectedCase" class="bta-panel">
        ...
        <div class="bta-panel-bd">
          ...
          <div class="comment-section">
            ...
            <textarea [(ngModel)]="comment" class="bta-textarea" rows="4" placeholder="Enter your comment..."></textarea>
          </div>
          <div class="bta-actions" style="margin-top:12px;">
            <button class="bta-btn bta-btn-secondary" (click)="selectedCase=null">Back to List</button>
            <button class="bta-btn bta-btn-primary" (click)="submitComment()">Submit Comment</button>
            <button class="bta-btn bta-btn-danger" (click)="closeCase()">Close Case</button>
          </div>
        </div>
      </div>

      <!-- Search + List — matches document image27 -->
      <div *ngIf="!selectedCase" class="bta-panel">
        <div class="bta-panel-hd">BTA TMC Worklist</div>
        <div class="bta-panel-bd">
          <div class="search-row">
            <label>Search Cases</label>
            <input type="text" [(ngModel)]="searchTerm" class="bta-input" placeholder="Enter Case ID or keyword"/>
          ...
          <div *ngIf="searched && filteredCases.length === 0" class="no-records">No cases found</div>

          <table *ngIf="filteredCases.length" class="bta-table" style="margin-top:14px;">
            <thead>
  ...
  styles: [`
    ...
    .bta-textarea     { width:100%; padding:8px; font-size:12px; font-family:Arial,sans-serif; border:1px solid #aaa; resize:vertical; margin-top:6px; }
...
export class BtaCaseManagementComponent {
  ...
  searchTerm = ''; searched = false; comment = '';
  ...
  search()  { this.searched = true; }
  clearSearch() { this.searchTerm=''; this.searched=false; }
  viewCase(c: BtaCase) { this.selectedCase = c; }
  goBack()  { this.selectedCase = null; }
  submitComment() { alert('Comment submitted.'); this.comment=''; }
  closeCase() { if (this.selectedCase) { this.selectedCase.status='Closed'; this.selectedCase=null; } }
}
